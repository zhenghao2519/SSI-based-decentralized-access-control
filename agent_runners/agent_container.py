...
trusted_issuers_did = ["FgXpwQjics4eiLwX1zXR9"]

# Here is an example Transformation function
transformation = {
    "empolyee schema": {
        "FgXpwQjics4eiLwX1zXR9": [
            {
                "title": "role",
                "role": "title"
            }
...
class AriesAgent(DemoAgent):
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "request-received":
            ...
            self.log("Proof =", proof["verified"])
            self.log("Verification Response =", proof)
    ...
    async def handle_basicmessages(self, message):
        try:
            ...
            if (msg_type != 'service_request'):
                ...
                requested_permission_validators = rsp_decoded['result']['requested_roles']
                self.log("Requested permission validator for the given object and action:",
                         rsp_decoded['result']['requested_roles'])
                # store the request with the connection in storage
                service_request = {connection_id: {"object": requested_object, "action": requested_action}}
                service_request_storage.append(service_request)
                req_attrs = []
                req_preds = []
                jsondata = JsonSearch(object=transformation, mode='j')
                all_path = jsondata.search_all_path(key='role')
                all_value = jsondata.search_all_value(key='role')
                for i in range(0, len(all_path)):
                    req_attrs.append({"name": all_value[i], "restrictions": [{"schema_name": all_path[i][0]},
                                                                             {"issuer_did": all_path[i][1]}]})
                ...
                proof_request_response = await self.agent.admin_POST(
                    "/present-proof-2.0/send-request", final_proof_request
                )
                self.log("Response of the request", proof_request_response)
...
class AgentContainer:
    ...
    async def initialize(
        ...
        if (not self.endorser_role) or (self.endorser_role == "endorser"):
            if (not self.agent.loading_wallet) and self.public_did and self.cred_type != CRED_FORMAT_JSON_LD:
