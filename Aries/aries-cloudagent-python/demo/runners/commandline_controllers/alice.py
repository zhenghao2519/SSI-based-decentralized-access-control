    ...
    try:
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
                ...
                # Send a service request using basic message
                requested_object = await prompt("Enter the object of the request: ")
                requested_action = await prompt("Enter the action of the request: ")
                service_request = {
                ...
                if service_request_msg:
                    await alice_agent.agent.admin_POST(
                        f"/connections/{alice_agent.agent.connection_id}/send-message",
                        {"content": service_request_msg},
                    )
